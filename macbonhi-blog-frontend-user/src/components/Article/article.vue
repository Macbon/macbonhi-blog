  ...
  <div class="article-container">
    ...
    <div class="article-list" v-if="articleList.length > 0">
      <div v-for="item in articleList" :key="item.id" class="article-item-wrapper">
        <ArticleItem :data="item" @click="showArticleDetail(item)" />
      </div>
    </div>
...
import { getLabelApi, getBatchArticlePraiseStatusApi, updateArticleViewsApi, getArticleCommentsApi } from '../../api/index';
...
// 获取文章数据 (重命名为避免与hook冲突)
const loadArticles = async (append = false) => {
  console.log('📡 Article: 调用 fetchArticles...', requestParams, append);
  ...
  try {
    ...
    articleList.value.forEach(article => {
      ...
      // 初始化评论数量到全局store
      commentStore.setCommentCount(article.id, article.comment || article.comments || 0);
    });
    
    // 提取文章ID列表
    const articleIds = articleList.value.map(article => article.id);
    
    // ✅ 临时跳过批量获取点赞状态API调用（该API不存在）
    console.log('⚠️ 跳过批量点赞状态API调用（接口不存在）');
    
    // 直接使用本地存储的点赞状态
    articleList.value.forEach(article => {
      const isLocalPraised = hasPraisedItem(0, article.id);
      praiseStore.setPraiseState(article.id, article.praise_count || 0, isLocalPraised);
    });
  } catch (error) {
