...
import { addSubsetApi, getSubsetApi, updateSubsetApi, deleteSubsetApi, getArticleStateApi, getDownloadableClassifyCountApi } from '../api/index';
...
export function useSubset(emit?: EmitFn, isDownloadPage?: boolean) {
    // 在函数内部初始化 store
    ...
    const rawSubset = (e:number) => {
        // 下载页面使用专门的API获取可下载文件分类（排除图片）
        if (e === 2 && (isDownloadPage || window.location.pathname.includes('/files'))) {
            let request = {
                token: userStore.token,
                classify: e
            }

            getDownloadableClassifyCountApi(request).then((res: any) => {
                if (tackleCode(res.code)) {
                    // 使用新方法按类型存储分类数据
                    subsetStore.updateSubsetsByType(e, res.data.list, res.data.count);
                    // 更新表格数据
                    dataSource.value = initData();
                }
            })
